...
4. Run the companion helper exactly once, passing the exact returned URL. It must serialize concurrent launches, then select and reuse an existing workspace or home-page tab before it may create a new tab. It may retire only installation tabs marked `installation_state=completed` or `installation_state=superseded`, and may reuse one of those terminal tabs when no workspace or home page exists. It must never repurpose an unconfirmed installation, account, or legal page or close a browser window by removing its sole tab:
