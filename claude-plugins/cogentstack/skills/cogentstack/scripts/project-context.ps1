...
function Test-CogentStackContextKey([string]$Value) { return $Value -eq 'default' -or $Value -match '^ctx-[0-9a-f]{64}$' }
function Get-CogentStackProjectContext([string]$ExplicitContextKey = '', [string]$WorkingDirectory = (Get-Location).Path) {
    ...
    $projectIdentifier = ''
    $source = ''
    if ($env:CODEX_PROJECT_ID -match '^g-p-[0-9a-f]{32}$') { $projectIdentifier = $env:CODEX_PROJECT_ID; $source = 'codex-project-environment' }
    elseif ($WorkingDirectory -match '(?i)[\\/]\.codex[\\/]\.chatgpt-projects[\\/](g-p-[0-9a-f]{32})(?:[\\/]|$)') { $projectIdentifier = $Matches[1]; $source = 'chatgpt-project-worktree' }
    if (-not $projectIdentifier) { return [pscustomobject]@{ ContextKey = 'default'; Source = 'legacy-default'; Isolated = $false } }
