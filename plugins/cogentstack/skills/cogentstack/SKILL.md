...
When the user asks to hide or close the companion, run `scripts/open-cogentstack-companion.ps1 -Mode Hide` or `-Mode Close`. The helper clears the temporary browser crop, restores the remembered ChatGPT or Codex position plus the reused browser's original window style and position, removes or hides the white backdrop, and never closes the user's reused browser window.
