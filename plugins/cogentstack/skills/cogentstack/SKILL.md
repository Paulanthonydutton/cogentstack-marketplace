...
When the user selects the companion-only X in the workspace header, the page navigates that reused tab to the CogentStack home page and the launcher's local exit watcher performs the same reversible restoration automatically. When the user asks in the adjacent composer to hide or close the companion, run `scripts/open-cogentstack-companion.ps1 -Mode Hide` or `-Mode Close`. The helper clears the temporary browser crop, restores the remembered ChatGPT or Codex position plus the reused browser's original window style and position, removes or hides the white backdrop, and never closes the user's reused browser window.
