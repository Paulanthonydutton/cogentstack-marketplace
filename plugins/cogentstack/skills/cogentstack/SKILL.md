...
5. Immediately run `scripts/open-cogentstack-companion.ps1 -Mode Open -Url <exact returned URL>` exactly once. This selects the exact existing CogentStack tab in Chrome or Edge and reuses its owning normal browser window. Only when no CogentStack tab exists may it open the exact URL once as a normal tab in the registered browser; it must never create a separate `--app` or `--new-window` session. The helper creates a white work-area backdrop at the bottom of the window stack, selects the foreground ChatGPT or Codex window, temporarily makes the reused browser frame borderless, positions its browser controls outside the work area, and applies a reversible window-region crop to the measured web document bounds so Chrome's retained edge is not visible. It then verifies an equal-width 50/50 split: ChatGPT or Codex on the left, the CogentStack page on the right, a 12-pixel white vertical divider, aligned top edges, and full work-area height. It must report `browserContentMode: page-only`, `browserChromeHidden: true`, `browserContentClipped: true`, `gutter: 12`, and `separated: true`; it must not send F11 or use monitor-wide browser fullscreen. Treat `arranged` with `layoutVerified: true` as fully positioned and `opened_unarranged` as not safely positioned. If it returns `browser_unavailable`, explain that Google Chrome or Microsoft Edge must be installed for the companion flow. Do not silently fall back to an embedded Codex panel.
...
When the user selects the companion-only X in the workspace header, the page navigates that reused tab to the CogentStack home page and the launcher's local exit watcher clears the temporary crop, restores the normal browser frame, maximizes that browser with Windows `SW_MAXIMIZE`, restores the remembered ChatGPT or Codex position, and removes the backdrop. This is ordinary window maximization, never browser F11 fullscreen. When the user asks in the adjacent composer to hide or close the companion, run `scripts/open-cogentstack-companion.ps1 -Mode Hide` or `-Mode Close`. Those explicit modes retain the reversible behavior: they clear the temporary browser crop, restore the remembered ChatGPT or Codex position plus the reused browser's original window style and position, remove or hide the white backdrop, and never close the user's reused browser window.
