...
The Codex plugin is installed from the Git-backed `.agents/plugins/` marketplace and retains `$cogentstack` as the common invocation. On Windows its default helper selects the exact existing `surface=chatgpt` tab in Chrome or Edge through Windows accessibility, reuses that signed-in page's owning normal browser session, hides the ChatGPT/Codex sidebar, and puts a white backdrop at the bottom of the Windows work area. It temporarily removes the reused browser frame, positions its controls outside the work area, and applies a reversible window-region crop to the measured CogentStack document so Chrome's retained frame inset is not visible. It then verifies an equal-width 50/50 workspace around a 12-pixel white divider, with full work-area height and aligned top edges. This reversible page-only layout does not use monitor-wide F11. The header presents **−**, **+**, and **X** in that order: minus collapses the split, plus restores it directly from the same page through the persistent local watcher, and X fully exits. Collapse, Suspend, or navigation to localhost restores the ordinary maximized browser while retaining a narrow local window record for fast Resume. A **CogentStack Work Mode (Codex)** Start-menu shortcut remains only as a fallback for that remembered arrangement. If the remembered windows or exact CogentStack workspace tab no longer exist, it refuses to guess and requires the normal `$cogentstack` cold start. Only when no matching tab exists does that full launch open the exact hosted URL once as a normal tab in the registered browser. It never launches a separate app-mode window, guesses a browser profile, reads or copies cookies, queues a native panel mount, or creates a duplicate after finding the existing tab.
