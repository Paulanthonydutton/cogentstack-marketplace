...
3. Use only Codex's trusted plugin manager to prepare the official Git-backed marketplace. Inspect the `cogentstack` registration once with `codex plugin marketplace list --json`. Its root must be a Git checkout whose `origin` is `https://github.com/Paulanthonydutton/cogentstack-marketplace.git` and whose sparse paths are exactly `.agents/plugins` and `plugins/cogentstack`.
   - If absent, run `codex plugin marketplace add https://github.com/Paulanthonydutton/cogentstack-marketplace.git --sparse .agents/plugins --sparse plugins/cogentstack --json` once.
...
4. Read the marketplace list once more, resolve the reported `cogentstack` root, and require the exact checked-in file `.agents/plugins/install-cogentstack.ps1` beneath that root. Do not search anywhere else or copy the file.
...
The installer verifies `https://cogentstack.app/stack?surface=chatgpt`, the prepared Git remote and sparse paths, plugin installation and enabled state, version parity, exact public-file allowlist and source hashes, launcher safeguards, and the one permitted account-bound claim. Its process-owned timeout stops a running child command; it does not measure or make claims about Codex reasoning time.
